...
// Given a csv file, return the minimum of the specified column, excluding
// blanks return -1.0 if the column is optional return -2.0 if the column does
// not exist
double getMin(char csvfile[], char column[]);

// Given a csv file, return the maximum of the specified column, excluding
// blanks return -1.0 if the column is optional return -2.0 if the column does
// not exist
double getMax(char csvfile[], char column[]);

// Given a csv file, return the average of the specified column, excluding
// blanks return -1.0 if the column is optional return -2.0 if the column does
// not exist
double getAvg(char csvfile[], char column[]);

// Given a csv file, return the number of students with their column value >=
// threshold, excluding blanks return -1 if the column is optional return -2 if
...
void printHelp() {
    ...
    printf("\tmin column\n");
    printf(
        "\t*** find the minimum of the specified column, excluding blanks\n");
    printf("\t*** for example: min P5.6\n");
    printf("\tmax column\n");
    printf(
        "\t*** find the maximum of the specified column, excluding blanks\n");
    printf("\t*** for example: max C5.2\n");
    printf("\tavg column\n");
    printf(
        "\t*** find the average of the specified column, excluding blanks\n");
    printf("\t*** for example: avg L5.26\n");
    printf("\tcount column threshold\n");
    printf(
        "\t*** find the number of rows with its column value >= threshold, "
        "excluding blanks\n");
    printf("\t*** for example: count P5.12 60\n");
    printf("\tgrade firstname lastname\n");
    printf(
        "\t*** find the weighted average of the specified student. A blank "
        "cell is viewed as 0\n");
    ...
    while (1) {
        ...
        if (strcmp(cmd, "grade") == 0) {
            ...
            if (isValidColumn(column)) {
                ...
                printf(
                    "A valid column name starts with a letter (P, C or L), "
                    "followed by a chapter number, a dot, and a section number "
                    "such as L5.25\n");
